...
class VAETrainer(BaseTrainer):
    ...
    def _update_kl_weight(self) -> None:
        ...
        self.current_kl_weight = min(normalized_position * 2.0 * self.max_kl_weight, self.max_kl_weight)
            
        print(f"Epoch {self.current_epoch+1}/{self.num_epochs} - KL weight: {self.current_kl_weight:.6f}")
    ...
    def train_epoch(self, train_loader: torch.utils.data.DataLoader) -> float:
        ...
        for data, _ in tqdm(train_loader, desc="Training", leave=False):
            ...
            recon_loss = F.binary_cross_entropy(flattened_recon, flattened_data, reduction='sum') / data.size(0)
        ...
        print(f"Train - Recon Loss: {avg_recon_loss:.4f}, KL Loss: {avg_kl_loss:.4f}, Weighted KL: {avg_weighted_kl_loss:.4f}")
    ...
    def validate(self, val_loader: torch.utils.data.DataLoader) -> float:
        ...
        print(f"Val - Recon Loss: {avg_recon_loss:.4f}, KL Loss: {avg_kl_loss:.4f}, Weighted KL: {avg_weighted_kl_loss:.4f}")
